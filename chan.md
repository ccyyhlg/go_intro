...
`chansend`函数的内部，是一系列精心设计的`if-else`分支，它，覆盖了向`channel`发送数据时，所有可能遇到的情况。
...
与`chansend`类似，新版的`chanrecv`函数，在保持原有核心逻辑不变的基础上，通过，增加无锁路径、优化锁持有时间等手段，变得更加高效和精密。
...
-   **意义**: 这个“取头，放尾”的设计，极其巧妙地，**同时，保证了两种公平**：
    -   **数据FIFO**: 接收者，总是，拿到缓冲区中，最旧的数据。
    -   **Goroutine先到先服务**: 等待最久的发送者，总是，最先，得到服务。
